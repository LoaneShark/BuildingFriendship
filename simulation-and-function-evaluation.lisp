...
;; arg key, and returns a list of key plus all the generated hashkeys.
;; Example 1: 
...
(defun add_tuple_to_hashtable (tuple table to-collect-terms)
	(let ( (keys (generate_allkeys_from_hashkey (convert_pred_to_hashkey tuple)))
		   new-hash-value old-hash-value  all-terms
		   (ct 0)
		 )
		...
		(dolist (key keys)
			...
			(if (not (null old-hash-value))
				(prog2
					(setq new-hash-value (unionf (list tuple) (cdr old-hash-value)))
					(when (and (equal (car new-hash-value) tuple) (not (equal tuple (second old-hash-value))))
						(setf (gethash key table) (cons (+ 1 (car old-hash-value)) new-hash-value))
				...
				(prog2
					(setf (gethash key table) (list 1 tuple))
					(when (eq to-collect-terms 'T)
						(incf ct 1)
					)
				)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; This function adds the list-of-wffs arg list-of-tuples to the hashtable 
;; arg table, also collecting and returning a list of all terms appearing 
;; all those tuples in list-of-tuples newly added to table (not already in
;; table) if input arg to-collect-terms is 'T. Otherwise, no terms are 
	...
	(let (list-of-terms-in-added-tuples)
		(dolist (tuple list-of-tuples)
			(setq list-of-terms-in-added-tuples 
				(merge_term_list_with_term_list (add_tuple_to_hashtable tuple table to-collect-terms)
...
(defun merge_term_list_with_term_list (lst1 lst2)
	(when (null lst1) (return-from merge_term_list_with_term_list lst2))
	(when (null lst2) (return-from merge_term_list_with_term_list lst1))
		...
		(dolist (pair lst1)
			...
			(if (numberp index)
				(prog2
					...
					(setq resulting_lst (substitute (list (+ (first found_pair) count_of_term) the_term) found_pair resulting_lst))
				)
				(prog2
					(setq resulting_lst (cons pair resulting_lst))
